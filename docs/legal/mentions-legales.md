# Mentions légales

**Dernière mise à jour : [À COMPLÉTER : date de mise en ligne]**

## 1. Éditeur du service

Le service DjiguiFlow, accessible à l'adresse <https://www.djiguiflow.com>, est édité par :

- **Exploitant** : OULAÏ Jean Paul
- **Forme** : entreprise individuelle
- **Adresse** : [À COMPLÉTER : adresse complète — commune, quartier, Abidjan / Côte d'Ivoire]
- **Numéro de registre** : RCCM CI-AGB-2021-A-307
- **Numéro de compte contribuable** : [À COMPLÉTER, ou supprimer cette ligne]
- **Téléphone** : +225 01 02 91 88 86
- **Courriel** : contact@djiguiflow.com

## 2. Directeur de la publication

OULAÏ Jean Paul, en qualité d'exploitant.

## 3. Hébergement

Le service est hébergé et exploité au moyen des prestataires suivants :

| Rôle | Prestataire |
|---|---|
| Hébergement de l'application web | Vercel Inc. — 440 N Barranca Ave #4133, Covina, CA 91723, États-Unis |
| Base de données et authentification | Supabase Inc. — 970 Toa Payoh North, Singapour |
| Automatisation des traitements | Serveur privé virtuel — Hostinger International Limited, Paris, France |

La liste complète des prestataires qui traitent des données personnelles, et les garanties associées, figure dans la [Politique de confidentialité](politique-confidentialite.md).

## 4. Contact

- Questions générales et commerciales : contact@djiguiflow.com
- Support technique : support@djiguiflow.com
- Protection des données : [À COMPLÉTER : adresse dédiée, par exemple donnees@djiguiflow.com]

## 5. Propriété intellectuelle

La dénomination « DjiguiFlow », le logo, la charte graphique, les textes, l'architecture du service et le code source qui le compose sont la propriété exclusive de l'éditeur, à l'exception des composants tiers utilisés sous leurs licences respectives.

Les contenus publiés par les marchands — nom et description de la boutique, catalogue, photographies de produits — demeurent la propriété de ces derniers. Les photographies retravaillées par le service pour la vitrine restent la propriété du marchand qui les a fournies.

## 6. Signalement d'un contenu illicite

Toute personne constatant la présence sur le service d'un contenu manifestement illicite peut le signaler à contact@djiguiflow.com, en précisant l'adresse exacte de la page concernée et le motif du signalement. L'éditeur retire sans délai les contenus manifestement illicites qui lui sont signalés.

## 7. Droit applicable

Les présentes mentions légales sont régies par le droit ivoirien.

---

> **Note de rédaction — à supprimer avant publication.**
> Ce document est un projet. Il doit être relu par un conseil juridique ivoirien avant mise en ligne, et les mentions `[À COMPLÉTER]` renseignées. Les références légales citées dans l'ensemble des documents juridiques du service doivent être vérifiées dans leur version en vigueur.
